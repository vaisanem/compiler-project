...
def parse(tokens: list[Token]) -> ast.Expression: #rename pos to index?
    #TODO: refactor and add tests
    pos = 0
    
    def peek() -> Token:
        if not pos < len(tokens):
            return Token(Type.END, "", tokens[-1].position) #Type = null?
        return tokens[pos]
    ...
    def parse_factor() -> ast.Expression:
        ...
        raise Exception(f'{peek().position}: expected your mum')
